...
export default function TrackingPage() {
  ...
  // Handle Search - Fetch dari API
  const handleSearch = async (e) => {
    e.preventDefault();
    
    if (!orderId || orderId.trim() === "") {
      toast.warning("Masukkan Project ID terlebih dahulu");
      return;
    }
      ...
      if (response.data.success) {
        ...
        const transformedProject = {
          ...
          timeline: project.timeline.map(item => ({
            ...
            title: item.title,
            date: item.date,
            status: item.status, // completed, current, pending
            note: item.note
          }))
        };
        
        setProjectData(transformedProject);
        toast.success("Project ditemukan! 🎉");
      }
    } catch (err) {
  ...
  return (
    <section className="min-h-screen bg-godinov text-white flex flex-col items-center justify-center p-6 pt-24 relative overflow-hidden">
      ...
      >
        ...
        <motion.div 
          layout="position" 
          className={`p-10 flex flex-col justify-center relative z-20 ${
            projectData ? "lg:col-span-1 border-b lg:border-b-0 lg:border-r border-white/10" : "w-full"
          }`}
        >
          ...
          <div className="relative z-10">
              ...
              <form onSubmit={handleSearch} className="space-y-6">
                <div className="relative group">
                  <input 
                    ...
                    className="w-full bg-cyan/40 border border-white/20 text-godinov-cyan placeholder-white/20 px-4 py-4 rounded-xl focus:outline-none focus:border-godinov-cyan focus:shadow-[0_0_15px_rgba(0,255,255,0.2)] transition-all font-poppins tracking-wider uppercase"
                    disabled={loading}
                  />
                ...
                <button 
                  type="submit" 
                  disabled={loading}
                  className="w-full border border-godinov-cyan bg-godinov-cyan text-white font-bold py-4 rounded-xl hover:bg-white hover:text-black transition-all duration-300 flex items-center justify-center gap-2 relative overflow-hidden group cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <span className="relative z-10">
                    {loading ? "Mencari Project..." : "Lacak Project"}
                  </span>
              ...
              >
                <div className="space-y-4 font-Arial text-sm">
                  <div className="bg-black/40 p-4 rounded-lg border border-white/10">
                    <p className="text-godinov-cyan text-xs mb-1 uppercase tracking-wider">Client</p>
                    <p className="text-white text-lg font-semibold">{projectData.client}</p>
                  </div>
                  
                  <div className="bg-black/40 p-4 rounded-lg border border-white/10">
                    <p className="text-godinov-cyan text-xs mb-1 uppercase tracking-wider">Tipe Project</p>
                    <p className="text-white">{projectData.service}</p>
                  </div>
                  
                  <div className="bg-black/40 p-4 rounded-lg border border-white/10">
                    <p className="text-godinov-cyan text-xs mb-1 uppercase tracking-wider">Status Saat Ini</p>
                  ...
                  <div className="bg-black/40 p-4 rounded-lg border border-white/10">
                    <p className="text-godinov-cyan text-xs mb-1 uppercase tracking-wider">Project ID</p>
                    <p className="text-white font-mono text-sm">{projectData.id}</p>
                  </div>
                </div>
                
                <button 
                  onClick={handleReset}
                  className="mt-8 text-sm text-white/40 hover:text-white transition-colors w-full text-center py-2 border border-white/10 rounded-lg hover:border-white/30"
                >
                  ← Cari ID Lain
                </button>
              </motion.div>
            ...
            {error && (
              ...
              >
                ...
                <span>ID Project tidak ditemukan di sistem</span>
              </motion.div>
            )}
          </div>
        </motion.div>
                ...
                <div className="max-w-xl mx-auto pt-4 pb-20">
                  {projectData.timeline && projectData.timeline.length > 0 ? (
                    ...
                    <div className="text-center py-10">
                      <p className="text-white/50">Belum ada timeline untuk project ini</p>
                    </div>
                  )}
                </div>
              </div>
